...
type CreaturesView = 'menu' | 'monsters' | 'npcs' | 'spells' | 'items';

const PAGE_SIZE_OPTIONS = [25, 50, 100, 200];
const DEFAULT_PAGE_SIZE = 50;
const SEARCH_DEBOUNCE_MS = 350;

const MENU_CARDS: { view: CreaturesView; icon: React.ReactNode; title: string; description: string }[] = [
  { view: 'monsters', icon: <PetsIcon sx={{ fontSize: 40 }} color="primary" />, title: 'Monsters', description: 'Vanilla stat blocks' },
  { view: 'npcs', icon: <BadgeIcon sx={{ fontSize: 40 }} color="primary" />, title: 'NPCs', description: 'People in the campaign' },
...
export function CreaturesSection({ campaignId }: CreaturesSectionProps) {
  const [view, setView] = useState<CreaturesView>('menu');
  const [filterOpen, setFilterOpen] = useState(false);
  const [search, setSearch] = useState('');
  const debouncedSearch = useDebouncedValue(search, SEARCH_DEBOUNCE_MS);

  // Shared across all 4 browse views - only one is ever visible at a time.
  ...
  useEffect(() => {
    if (view !== 'monsters' && view !== 'npcs') return;
    const category: CreatureCategory = view === 'monsters' ? 'monster' : 'npc';
    fetchCreatureBrowse({
      campaignId,
      category,
      scope,
  ...
  const goToView = (next: CreaturesView) => {
    setView(next);
    setFilterOpen(false);
    setSearch('');
    setPage(0);
  ...
  if (view === 'monsters' || view === 'npcs') {
    ...
    const monsterFiltersActive =
      ...
      crMinFilter !== '' ||
      crMaxFilter !== '';
    const npcFiltersActive = relationFilter.length > 0 || importanceFilter.length > 0;
    const hasActiveFilters = view === 'monsters' ? monsterFiltersActive : npcFiltersActive;
    const isFiltered = !!debouncedSearch || hasActiveFilters;
    return (
      <Box>
        <SectionHeader
          title={view === 'monsters' ? 'Monsters' : 'NPCs'}
          onBack={() => goToView('menu')}
          addLabel={view === 'monsters' ? 'Add Monster' : 'Add NPC'}
        ...
        {filterOpen && (
          <FilterBar
            ...
            searchPlaceholder={`Search ${view} by name or traits…`}
        ...
        {creatureBrowseLoading && items.length === 0 ? (
          ...
          <EmptyState
            icon={category === 'monster' ? <PetsIcon sx={{ fontSize: 56, color: 'text.disabled' }} /> : <BadgeIcon sx={{ fontSize: 56, color: 'text.disabled' }} />}
            title={`No ${view} yet`}
        ...
        <ConfirmDeleteDialog
          ...
          itemType={category === 'monster' ? 'monster' : 'NPC'}
